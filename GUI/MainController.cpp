...
#include <GUI/Tools/MujocoReader.h>
#include <opencv2/core/ocl.hpp>
#include "Tools/rostopicReader.h"
#include <toml.hpp>
...
MainController::MainController(int argc, char* argv[])
    ...
    chatter_pub = n.advertise<std_msgs::String>("/maskfusion/lwh_xyz", 1000);
    ...
    Parse::get().arg(argc, argv, "-mj", logFile);
    if (logFile.length()) {
        logReader = std::make_unique<MujocoReader>(logFile, Parse::get().arg(argc, argv, "-f", tmpString) > -1);
        logReaderReady = true;
        
    }

    // read rgb and depth from ros topic
    if (!logReaderReady) {
        Parse::get().arg(argc, argv, "-ros", logFile);   // read 3d information from ros topic
        if (logFile.length()) {  
            logReader = std::make_unique<rosropicLogReader>(logFile, Parse::get().arg(argc, argv, "-f", tmpString) > -1);
            logReaderReady = true;
        }
    }
...
void MainController::run() {
    std::cout<<"void MainController::run()"<<std::endl;
    while (!pangolin::ShouldQuit() && !((!logReader->hasMore()) && quit) && !(maskFusion->getTick() == end && quit)) {
        // std::cout<<"while:"<<gui->pause->Get()<<"||"<<pangolin::Pushed(*gui->step)<<std::endl;
        if (!gui->pause->Get() || pangolin::Pushed(*gui->step)) {
            ...
            if ((logReader->hasMore() || rewind) && maskFusion->getTick() < end) {
                ...
                cv::imwrite("logReader->getFrameData.jpg", logReader->getFrameData()->rgb);
                // std::cout<<"logReader->getFrameData.jpg"<<std::endl;
...
void MainController::drawScene(DRAW_COLOR_TYPE backgroundColor, DRAW_COLOR_TYPE objectColor) {
    ...
    if (false) {
        ...
        //std::cout<<"MainController.cpp|"<<gui->drawObjectModels->Get()<<std::endl;
        if (gui->drawObjectModels->Get()) {
            ...
            glColor3f(1,1,0); // Todo Change color of bounding box?
            //std::cout<<"MainController.cpp|"<<renderData.size()<<std::endl;
            for (size_t i=0; i < renderData.size(); i++) {
                ...
                std::cout<<"MainController.cpp|"<<gui->drawBoundingBoxes->Get()<<std::endl;
                std::cout<<"MainController.cpp|"<<oData.bb.min()(2)<<std::endl<<std::endl;
    ...
    if(true){
        ...
        if(tick != last_tick){
            for (size_t i=0; i < renderData.size(); i++) {
                ...
                std::stringstream ss;
                std_msgs::String msg;
                ss << maskFusion->getTick() << "\n" <<
                           "l,w,h:" << maxb(0)-minb(0) << " " << maxb(1)-minb(1) << " " << maxb(2)-minb(2) << " \n" <<
                           "centre:" << centerb(0) << ", " << centerb(1) << ", " << centerb(2) << "\n\n";
